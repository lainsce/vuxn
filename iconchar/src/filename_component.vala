...
public class FilenameComponent : Gtk.Box {
    ...
    public FilenameComponent(IconcharData data) {
    ...
    private void setup_ui() {
        ...
        filename_label = new Gtk.Label(char_data.filename);
        ...
        filename_entry.text = char_data.filename;
        ...
        char_data.notify["filename"].connect(() => {
            filename_label.set_text(char_data.filename);
            filename_entry.text = char_data.filename;
            
            // Parse dimensions if filename changes
            if (char_data.parse_dimensions_from_filename()) {
                // Dimensions changed, ensure UI is updated
                // This will be handled by data_changed signals
            }
        });
    }
    ...
    private void update_flash() {
        ...
        Gdk.RGBA fg_color = char_data.get_color(flash_state ? 1 : 0);
        Gdk.RGBA bg_color = char_data.get_color(flash_state ? 0 : 1);
    ...
    private void apply_edit() {
        ...
        // Ensure filename is not empty and has appropriate extension
        if (new_filename == "") {
            new_filename = "untitled10x10.chr";
        } else if (!new_filename.has_suffix(".chr") && !new_filename.has_suffix(".icn")) {
            new_filename += ".chr"; // Default to CHR extension
        }
        
        // Set the new filename
        char_data.filename = new_filename;
    }
