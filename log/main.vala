...
public class LogApp : Gtk.Application {
    ...
    private void create_content_area() {
        ...
        // Apply theme colors using 2-bit color scheme
        ...
        var css = @"textview {
            background-color: rgb($(bg_color.red*255),$(bg_color.green*255),$(bg_color.blue*255));
            color: rgb($(fg_color.red*255),$(fg_color.green*255),$(fg_color.blue*255));
            padding: 8px;
            font-family: 'Log';
            font-size: 16px;
        }";
        ...
        theme.theme_changed.connect(() => {
            // Update the CSS when theme changes
            Gdk.RGBA new_bg = theme.get_color("theme_bg");
            Gdk.RGBA new_fg = theme.get_color("theme_fg");
            
            var new_css = @"textview {
                background-color: rgb($(new_bg.red*255),$(new_bg.green*255),$(new_bg.blue*255));
                color: rgb($(new_fg.red*255),$(new_fg.green*255),$(new_fg.blue*255));
                padding: 8px;
                font-family: 'Log';
                font-size: 16px;
            }";
            
            provider.load_from_data(new_css.data);
        });
        
        // Get buffer
        buffer = text_view.get_buffer();
        
        // Create text tags for formatting using 2-bit color scheme
        Gdk.RGBA fg = theme.get_color("theme_fg");
        Gdk.RGBA accent = theme.get_color("theme_accent");
        Gdk.RGBA selection = theme.get_color("theme_selection");
        
        // Convert theme colors to hex format for tags
        string fg_hex = "#%02x%02x%02x".printf(
            (uint8)Math.round(fg.red*255),
            (uint8)Math.round(fg.green*255),
            (uint8)Math.round(fg.blue*255)
        );
        string accent_hex = "#%02x%02x%02x".printf(
            (uint8)Math.round(accent.red*255),
            (uint8)Math.round(accent.green*255),
            (uint8)Math.round(accent.blue*255)
        );
        string selection_hex = "#%02x%02x%02x".printf(
            (uint8)Math.round(selection.red*255),
            (uint8)Math.round(selection.green*255),
            (uint8)Math.round(selection.blue*255)
        );
        // Create tags with theme colors
        buffer.create_tag("system", "foreground", fg_hex);
        buffer.create_tag("error", "foreground", accent_hex);
        buffer.create_tag("hex", "foreground", fg_hex);
        buffer.create_tag("clipboard", "foreground", fg_hex);
    ...
    private void display_hex_content(uint8[] data) {
        StringBuilder hex_output = new StringBuilder();
        int column = 0;
        
        for (int i = 0; i < data.length; i++) {
            hex_output.append_printf("%02x ", data[i]);
        ...
        // End with newline if we have a partial line
        if (column > 0) {
            hex_output.append_c('\n');
        }
        
        append_text(hex_output.str, "hex");
    }
    ...
    private void paste_from_clipboard() {
        ...
        clipboard.read_value_async(typeof(string), 0, null, (obj, async_result) => {
            try {
    ...
    private void append_text(string text, string? tag = null) {
        ...
        if (tag != null) {
            buffer.insert_with_tags_by_name(ref end, text, -1, tag);
        } else {
    ...
    private void scroll_to_bottom() {
        Idle.add(() => {
            ...
            text_view.scroll_to_mark(buffer.get_insert(), 0.0, true, 0.0, 1.0);
            return false;
        });
    }
