// Optimized AsciiView to replace individual ASCII cell widgets
public class AsciiView : Gtk.Box {
    private Gtk.DrawingArea drawing_area;
    private uint8[] buffer;
    private int bytes_per_line = 16;
    private int byte_width = 12;  // Width of each ASCII cell (narrower than hex)
    private int byte_height = 20; // Height of each cell
    ...
    // Constructor
    public AsciiView(uint8[] buffer_ref, Gtk.Adjustment vadj, Window parent) {
        ...
        // Create drawing area for ASCII view
        drawing_area = new Gtk.DrawingArea() {
            vexpand = true,
            hexpand = true
        };
        drawing_area.set_draw_func(draw_ascii_view);
    ...
    // Drawing function
    private void draw_ascii_view(Gtk.DrawingArea area, Cairo.Context cr, int width, int height) {
        ...
        for (int row = first_visible_row; row < first_visible_row + visible_rows; row++) {
            for (int col = 0; col < bytes_per_line; col++) {
                ...
                if (byte_pos < buffer.length) {
                    ...
                    char c = (char)buffer[byte_pos];
                    string display_char = (c >= 32 && c <= 126) ? c.to_string() : ".";
                    cr.show_text(display_char);
    ...
    public void start_editing(int byte_pos) {
        ...
        if (edit_entry == null) {
            ...
            edit_entry.add_css_class("ascii-edit");
            ...
            entry_key.key_pressed.connect((keyval, keycode, state) => {
                if (keyval == Gdk.Key.Escape) {
                    ...
                    // Reset to "." (ASCII 46) when backspace is pressed
                    edit_entry.text = ".";
                    commit_edit();
                    return true;
                }
    ...
    private void commit_edit() {
        ...
        string text = edit_entry.text;
        if (text.length > 0) {
            char c = text[0];
            uint8 val = (uint8)c;
            
            // Update the buffer
            buffer[editing_byte] = val;
            
            // Notify the change
            if (byte_changed_callback != null) {
                byte_changed_callback(editing_byte, val);
            }
